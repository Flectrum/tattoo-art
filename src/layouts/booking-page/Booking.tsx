              ...
              <div className="flex mt-5 text-sm gap-4">
                {steps.map((step, index) => (
                  <div key={index}>
                    {step >= currentStep ? (
                      <div
                        className={`relative flex justify-center items-center text-xs w-6 h-6 font-semibold rounded-full ${step <= currentStep ? "bg-red-800 text-white" : "bg-[#141414] text-muted"}`}
                      >
                        {step}
                      </div>
                    ) : (
